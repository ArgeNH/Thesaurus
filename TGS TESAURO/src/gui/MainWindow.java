...
public class MainWindow extends JFrame implements ActionsThesaurus{
	...
	public MainWindow(){
		setTitle("TGS ~ Tesauro | Arge Ni�o");
	...
	public void writeOuput(String option, String[] output, boolean state) {
		...
		switch(option){
			case ActionsThesaurus.ADD_WORD:
				if(state==true){
					JOptionPane.showMessageDialog(null, "Se agrego el t�rmino, con exito");
					cleanFields();
				}else{
					JOptionPane.showMessageDialog(null, "Este t�rmino ya existe, o no se puede agregar");
				}
				break;
			case ActionsThesaurus.FIND_WORD:
				if(state==true){
					JOptionPane.showMessageDialog(null, "T�rmino encontrado con exito");
